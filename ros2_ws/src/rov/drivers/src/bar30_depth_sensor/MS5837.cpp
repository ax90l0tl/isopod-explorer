...
{
	pi_ = pigpio_id;
	handle_ = i2c_open(pi_, I2C_BUS, I2C_ADDR, 0);
	if (handle_ < 0)
	{
		cout << handle_ << "Failed to open i2c line" << endl;
		return;
	}
	setOSR(5);
...
bool MS5837::init()
{
	...
	for (int i = 0; i < 7; i++)
	{
		temp = i2c_read_word_data(pi_, handle_, MS5837_PROM_READ + i * 2);
	...
	// Set model_ according to the sensor version
...
void MS5837::read()
{
	...
	while (!write(MS5837_CONVERT_D1_256 + (2 * OSR_)))
	{
		this_thread::sleep_for(chrono::milliseconds(100));
	}

	// float time = get_current_tick(pi_) / (1e6);
	...
	this_thread::sleep_for(chrono::microseconds(int(2.5 * pow(2, (8 + OSR_)))));

	D1_pres = 0;
	char p_buff[3];
	while (i2c_read_i2c_block_data(pi_, handle_, MS5837_ADC_READ, p_buff, 3) < 0)
		;

	// time = get_current_tick(pi_) / (1e6) - time;
	// cout << "that took: " << dec << time << " sec" << endl;
	uint8_t *p_buff_int = (uint8_t *)p_buff;
	D1_pres = p_buff_int[0] << 16 | p_buff_int[1] << 8 | p_buff_int[2];

	// Request D2 conversion
	while (!write(MS5837_CONVERT_D2_256 + (2 * OSR_)))
	{
		this_thread::sleep_for(chrono::milliseconds(100));
	}
	...
	D2_temp = 0;
	char t_buff[3];
	while (i2c_read_i2c_block_data(pi_, handle_, MS5837_ADC_READ, t_buff, 3) < 0)
		;
...
bool MS5837::write(uint8_t msg)
{
	...
	if (i2c_write_byte(pi_, handle_, msg_char) != 0)
	{
